...
///
/// `zap init` generates a commented template for both levels (project or global).
///
...
impl TomlFilterRegistry {
    ...
    fn load() -> Self {
                ...
                | crate::hooks::trust::TrustStatus::EnvOverride => {
                    if let Ok(content) = std::fs::read_to_string(project_filter_path) {
                        match Self::parse_and_compile(&content, "project") {
                            ...
                            Err(e) => eprintln!("[zap] warning: .rtk/filters.toml: {}", e),
                        }
                    }
                }
                crate::hooks::trust::TrustStatus::Untrusted => {
                    eprintln!("[zap] WARNING: untrusted project filters (.rtk/filters.toml)");
                    eprintln!("[zap] Filters NOT applied. Run `zap trust` to review and enable.");
                }
                crate::hooks::trust::TrustStatus::ContentChanged { .. } => {
                    eprintln!("[zap] WARNING: .rtk/filters.toml changed since trusted.");
                    eprintln!("[zap] Filters NOT applied. Run `zap trust` to re-review.");
                }
            }
        }
        ...
        if let Some(config_dir) = dirs::config_dir() {
            ...
            if let Ok(content) = std::fs::read_to_string(&global_path) {
                match Self::parse_and_compile(&content, "user-global") {
                    Ok(f) => filters.extend(f),
                    Err(e) => eprintln!("[zap] warning: {}: {}", global_path.display(), e),
                }
            }
        }
        ...
        match Self::parse_and_compile(builtin, "builtin") {
            Ok(f) => filters.extend(f),
            Err(e) => eprintln!("[zap] warning: builtin filters: {}", e),
        }
    ...
    fn parse_and_compile(content: &str, source: &str) -> Result<Vec<CompiledFilter>, String> {
        ...
        for (name, def) in file.filters {
            match compile_filter(name.clone(), def) {
                ...
                Err(e) => eprintln!("[zap] warning: filter '{}' in {}: {}", name, source, e),
            }
        }
        Ok(compiled)
    }
...
fn compile_filter(name: String, def: TomlFilterDef) -> Result<CompiledFilter, String> {
    ...
    for cmd in RUST_HANDLED_COMMANDS {
        if match_regex.is_match(cmd) {
            eprintln!(
                "[zap] warning: filter '{}' match_command matches '{}' which is already \
...
pub fn run_filter_tests(filter_name_opt: Option<&str>) -> VerifyResults {
    ...
    if project_path.exists() {
        ...
        match trust_status {
            ...
            _ => {
                eprintln!("[zap] WARNING: untrusted project filters skipped in verify");
            }
        }
    }
...
fn collect_test_outcomes(
    ...
    let file: TomlFilterFile = match toml::from_str(content) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("[zap] warning: TOML parse error during verify: {}", e);
            return;
        }
    };
    ...
    for (name, def) in file.filters {
        ...
        match compile_filter(name.clone(), def) {
            ...
            Err(e) => eprintln!("[zap] warning: filter '{}' compilation error: {}", name, e),
        }
    }
    ...
    for (filter_name, tests) in file.tests {
        ...
        let compiled = match compiled_filters.get(&filter_name) {
            ...
            None => {
                eprintln!(
                    "[zap] warning: [[tests.{}]] references unknown filter",
                    filter_name
                );
                continue;
            }

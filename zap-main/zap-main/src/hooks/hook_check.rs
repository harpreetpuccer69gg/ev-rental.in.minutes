...
/// Hook status for diagnostics and `zap gain`.
...
pub fn status() -> HookStatus {
    ...
    if binary_hook_registered(&claude_dir) {
        ...
        // (migration not complete — user should run `zap init -g` to clean up)
...
fn check_and_warn() -> Option<()> {
    let warning = match status() {
        ...
        HookStatus::Missing => {
            "[zap] /!\\ No hook installed — run `zap init -g` for automatic token savings"
        }
        HookStatus::Outdated => "[zap] /!\\ Hook outdated — run `zap init -g` to update",
    };
